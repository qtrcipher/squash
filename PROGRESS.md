...
> 3. Donations from day one vs wait — touches Phase 6 (still open).
> 4. User-created presets: docs 01/03 say "exactly 3 presets" but doc 06 models user presets — doc 06 proposal: GUI lists them, no preset editor in v1. Sign off or adjust (still open).
> 5. ~~TAP_GITHUB_TOKEN for tap auto-publish~~ — OWNER DECISION 2026-08-15: **skip**; Homebrew/Scoop publish jobs no-op by design, manifests updated manually if ever wanted.
...
- [x] v1.0.0 GitHub Release, announcement posts (OSS communities) (2026-08-15: **v1.0.0 PUBLISHED** — github.com/qtrcipher/squash/releases/tag/v1.0.0, 14 assets, checksums + updater manifest verified, shipped Linux CLI smoke-tested in Docker (round-trip byte-identical); announcement posts = owner's accounts, not done)

## Session log
Format, newest first, one line per session: `YYYY-MM-DD — what changed — next: <task>`
- 2026-08-15 — **v1.0.0 SHIPPED**: version bump (3 files + CHANGELOG), tag-triggered release run green (31846295728), release notes from CHANGELOG, published as latest; shipped Linux CLI verified in Docker (checksum match, tar.zst round-trip byte-identical, Arabic filenames); README Install now points at the release — next: announcements (owner), then v1.x maintenance per semver rules
- 2026-08-15 — v0.1.0 rehearsal: release pipeline proven end-to-end (4 legs green, signed updater manifest, SHA256SUMS; 4 bugs found+fixed: empty-cert codesign, ci-skip tag trap, Intel compiler-rt link, finalize GH_REPO); draft+tag deleted (rehearsal only). OWNER DECISION: **macOS target dropped** (signing embeds owner's legal name in binaries — privacy; reversible via org account) — supported targets now Windows + Linux; CI/release/packaging/docs updated, cfg'd macOS code retained but unsupported — next: cut v0.1.0/v1.0.0 when owner says go; remaining secrets: TAP_GITHUB_TOKEN, SQUASH_SENTRY_DSN, optional Windows cert + GPG
