...
- **Error taxonomy.** One `SquashError` enum, `thiserror`-based, with stable machine-readable codes (serialized into CLI `--json` and GUI error states): `UnsupportedFormat`, `CorruptArchive`, `PathTraversalBlocked`, `PermissionDenied`, `DiskFull`, `PasswordRequired` (stubbed, out of scope v1), `Cancelled`, `Internal`. Codes are a stability contract — majors only; Phase 3 added `DecompressionBomb` (additive, docs/07 §2).
...
- **Safety layer.** Extraction path sanitizer (zip-slip): every entry path canonicalized and verified inside the destination before a single byte is written. Lives in core, format-agnostic, cannot be bypassed by a handler. Phase 3 hardened it further (docs/07): link-target validation is physical (symlink-chain escapes rejected), writes never pass through existing symlinks, and a decompression-bomb guard (ratio/absolute/entry caps + rollback) bounds every extraction.
...
- **Fuzzing:** `cargo-fuzz` targets on every extraction entry point + the path sanitizer; seeded with `fixtures/`; run continuously (weekly CI cron, OSS-Fuzz eligibility post-launch). Given that incumbent RCEs drive user switching (doc 02 §2), the fuzz story is marketing as much as engineering. *Phase 5 status (landed):* the `fuzz/` crate (standalone workspace — nightly-only, excluded from stable `clippy --workspace`/`fmt --all` by design) has six targets driving the real `FormatHandler::extract` paths into per-run tempdirs with the bomb guard clamped via `SQUASH_EXTRACT_MAX_BYTES`/`SQUASH_EXTRACT_MAX_ENTRIES`: `fuzz_zip`, `fuzz_tar` (raw tar; also covers the tar.bz2/tar.xz container parse), `fuzz_tar_gz`, `fuzz_sevenz`, `fuzz_rar` (vendored UnRAR C++, ASan-instrumented via `CFLAGS`/`CXXFLAGS`), `fuzz_zst`. Seeds: `fuzz/seed-corpus.sh` (Squash-built archives mirroring the test tree + vendored rar fixtures) → `fuzz/corpus/`; magic-byte dictionary in `fuzz/dictionaries/archive.dict`. Run: `cargo +nightly fuzz run <target> -- -max_len=262144 -dict=dictionaries/archive.dict` from `fuzz/`. CI: `.github/workflows/fuzz.yml` — weekly cron + `workflow_dispatch`, 5 min per target on ubuntu, crashes uploaded as artifacts.
